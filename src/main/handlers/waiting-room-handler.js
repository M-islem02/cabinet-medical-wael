  ...
  ipcMain.handle('waiting-room:add', async (event, data) => {
    try {
      ...
      await run(`
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 'waiting')
      `, [id, data.patientId, data.arrivalTime, toNullIfEmpty(data.reason), toNullIfEmpty(data.notes), toNullIfEmpty(data.createdBy), toNullIfEmpty(data.assignedTo)]);
